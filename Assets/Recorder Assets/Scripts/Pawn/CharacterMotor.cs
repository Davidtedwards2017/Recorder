...
public class CharacterMotor : MonoBehaviour
{
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity)
    {
        ...
        if(grounded && jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2))
        {
            return PerformJump(velocity);
        }
        else if(!grounded && JumpsLeft > 0 && (Time.time - jumping.lastButtonDownTime < 0.2) && !jumping.holdingJumpButton )
        {
            JumpsLeft--;
            return PerformJump(velocity);
        }
    ...
    Vector3 PerformJump(Vector3 velocity)
    {
        grounded = false;
